  ...
  private async findAllFromExternalApi() {
    const response = await firstValueFrom(
      this.httpService.get(
        `${this.configService.get<string>('EXTERNAL_API_URL')}/api/sneakers?pagination%5Bpage%5D=1&pagination%5BpageSize%5D=100`,
      ),
    );
  ...
  private async checkIfExternalDataAreInDatabase() {
    try {
      ...
      if (totalItemInTable === 0) {
        ...
        const sneakersData = sneakerExternalResponse.data;
